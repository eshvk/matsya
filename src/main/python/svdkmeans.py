...
""" Wrapper script for LSA ( SVD + K Means).
    Performs Rank r (Default 250)  approximation of matrix. This is then
    clustered by KMeans. The output clusters are then visualized as follows:
    Generate concept vectors for each cluster, generate both feature clouds
...
def gen_args():
    ...
    This generator describes arguments necessary to take opinion data available
    https://wiki.mozilla.org/Firefox/Input/Data and perform LSA.
...
def main():
    """ Main entry point to script to generate LSA.
    """
    parser = gen_args()
